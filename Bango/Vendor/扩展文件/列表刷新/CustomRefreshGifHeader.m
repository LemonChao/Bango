...
- (UIView *)bgView {
    if (!_bgView) {
        ...
        for (int i = 0; i < 2; i ++) {
            UIImageView *image = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_refresh_bg"]];
...
- (UIImageView *)customImageV {
    if (!_customImageV ) {
        ...
        _customImageV.frame = CGRectMake(WidthRatio(200), 10, WidthRatio(120), HeightRatio(144));
...
- (UILabel *)sloganLabel
{
    if (!_sloganLabel ) {
        ...
        _sloganLabel.frame = CGRectMake(WidthRatio(325), 10, 200, 30);
        
        _sloganLabel.text = @"快节奏，漫生活";
        _sloganLabel.textColor = [UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1];
        _sloganLabel.font = [UIFont systemFontOfSize:HeightRatio(30)];
...
- (void)setPullingPercent:(CGFloat)pullingPercent {
    ...
    //需要动画的时候开启
//    self.customImageV.frame = CGRectMake(WidthRatio(222),  45 * (1 - pullingPercent), 30 * pullingPercent, 45 * pullingPercent);
...
////        weakSelf.customImageV.frame = CGRectMake(WidthRatio(222), 0, 30 * 0.9999, 45 * 0.9999);
////
...
- (UILabel *)customStatusLable {
    if (!_customStatusLable) {
        [self addSubview:_customStatusLable  = [[UILabel alloc] initWithFrame:CGRectMake(WidthRatio(325), 30, 100, 26)]];
        _customStatusLable.font = [UIFont systemFontOfSize:HeightRatio(22)];
